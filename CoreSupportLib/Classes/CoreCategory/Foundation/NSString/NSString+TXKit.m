...
- (BOOL)tx_isValidString {
    ...
    // 2. 判断是否是 非空白字符串
    if ([[self tx_stringByTrim] length] == 0) {
        return NO;
    }
    return YES;
}
...
- (CGFloat)tx_getTextWidth:(UIFont *)font height:(CGFloat)height {
    CGSize size = [self tx_getTextSize:font maxSize:CGSizeMake(MAXFLOAT, height) mode:NSLineBreakByWordWrapping];
    return size.width;
}

#pragma mark - 获取文本的高度
- (CGFloat)tx_getTextHeight:(UIFont *)font width:(CGFloat)width {
    CGSize size = [self tx_getTextSize:font maxSize:CGSizeMake(width, MAXFLOAT) mode:NSLineBreakByWordWrapping];
    return size.height;
}
...
- (BOOL)tx_isValidPhoneNumber {
    ...
    if (telNumber.length == 11) {
        ...
        if ([self tx_isValidateByRegex:CM_NUM] || [self tx_isValidateByRegex:CU_NUM] || [self tx_isValidateByRegex:CT_NUM]) {
            return YES;
        } else {
            return NO;
        }
...
- (BOOL)tx_isValidIPAddress {
    ...
    BOOL rc = [self tx_isValidateByRegex:regex];
    if (rc) {
...
- (NSNumber *)tx_getGenderFromIDCard {
...
- (NSString *)tx_hideCharacters:(NSUInteger)location length:(NSUInteger)length {
