...
@implementation UIImage (TXKit)

#pragma mark - 显示原图（避免被系统渲染成蓝色）
+ (UIImage *)tx_originalImage:(NSString *)imageName {
    return [[UIImage imageNamed:imageName] imageWithRenderingMode:UIImageRenderingModeAlwaysOriginal];
}

#pragma mark - 用颜色返回一张图片
+ (UIImage *)tx_imageWithColor:(UIColor *)color {
    return [self tx_imageWithColor:color size:CGSizeMake(1, 1)];
}

#pragma mark - 用颜色返回一张图片
+ (UIImage *)tx_imageWithColor:(UIColor *)color size:(CGSize)size {
...
- (UIImage *)tx_addFilter:(NSString *)filterName {
...
- (UIImage *)tx_alpha:(CGFloat)alpha {
...
#pragma mark - 设置圆角图片
- (UIImage *)tx_imageByRoundCornerRadius:(CGFloat)radius {
    return [self tx_imageByRoundCornerRadius:radius borderWidth:0 borderColor:nil];
}
...
- (UIImage *)tx_imageByRoundCornerRadius:(CGFloat)radius
    ...
    return [self tx_imageByRoundCornerRadius:radius
