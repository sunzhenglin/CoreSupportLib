    ...
    if (self.navigationBar.isTranslucent) {
        if (backgroundImageView != nil && [backgroundImageView isKindOfClass:[UIImageView class]] && backgroundImageView.image != nil) {
            ...
            if ([barBackgroundView isKindOfClass:NSClassFromString(@"_UIBarBackground")]) {
                ...
                UIView *backgroundEffectView = barBackgroundView.subviews.firstObject;// UIVisualEffectView
                if (backgroundEffectView != nil) {
                    backgroundEffectView.alpha = alpha;
                }
            }
...
- (void)et__updateInteractiveTransition:(CGFloat)percentComplete {
    ...
    if (topVC != nil) {
        ...
        if (coor != nil) {
            // 随着滑动的过程设置导航栏透明度渐变
            CGFloat fromAlpha = [[coor viewControllerForKey:UITransitionContextFromViewControllerKey].navigationAlpha floatValue];
            CGFloat toAlpha = [[coor viewControllerForKey:UITransitionContextToViewControllerKey].navigationAlpha floatValue];
...
- (void)dealInteractionChanges:(id<UIViewControllerTransitionCoordinatorContext>)context {
    if ([context isCancelled]) {// 自动取消了返回手势
        ...
        [UIView animateWithDuration:cancelDuration animations:^{
            CGFloat nowAlpha = [[context viewControllerForKey:UITransitionContextFromViewControllerKey].navigationAlpha floatValue];
            [self setNeedsNavigationBackground:nowAlpha];
        }];
    } else {// 自动完成了返回手势
        ...
        [UIView animateWithDuration:finishDuration animations:^{
            CGFloat nowAlpha = [[context viewControllerForKey:
                                 UITransitionContextToViewControllerKey].navigationAlpha floatValue];
            [self setNeedsNavigationBackground:nowAlpha];
        }];
    }
}
...
- (void)navigationBar:(UINavigationBar *)navigationBar didPopItem:(UINavigationItem *)item {
    if (self.viewControllers.count >= navigationBar.items.count) {// 点击返回按钮
        ...
        [self setNeedsNavigationBackground:[popToVC.navigationAlpha floatValue]];
    }
}

- (void)navigationBar:(UINavigationBar *)navigationBar didPushItem:(UINavigationItem *)item {
    // push到一个新界面
    [self setNeedsNavigationBackground:[self.topViewController.navigationAlpha floatValue]];
}
